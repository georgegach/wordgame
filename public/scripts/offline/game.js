...
Game.play = {
	...
	{
		Game.play.registerPlayer(new Game.Player(prompt("დასახელდი!", "სტუმარი"), "255,135,85"));
		// Game.play.registerPlayer(new Game.Player(localStorage.getItem('name'), "255,135,85"));
...
Game.front = 
{
	init : function()
	{
		...
		document.querySelector(".wordlist").classList.remove("hidden");
		document.querySelector(".exitIcon").classList.add("hidden");
...
Game.Words.prototype = 
{
	...
	validWords : function(letters, withoutAsterix)
	{
		var results = [];

		this._validWords(letters, "", results, this.trie, withoutAsterix);
	...
	_validWords : function(letters, word, results, trie, withoutAsterix)
	{
		...
		for (var i = 0; i < letters.length; i++) 
		{
			if (letters[i] == "*")
			{
				...
				{
					if (trie[this.alphabet[j]] != undefined)
					{
						if (trie[this.alphabet[j]].$)
						{
							// isWord
							if (withoutAsterix == true)
								results.push(word + this.alphabet[j]);
							else
								results.push(word + "*");
						}
						else
						{
							// isPrefix
							if (withoutAsterix == true)
								this._validWords(letters.pop(i), word + this.alphabet[j], results, trie[this.alphabet[j]], withoutAsterix);
							else
								this._validWords(letters.pop(i), word + "*", results, trie[this.alphabet[j]], withoutAsterix);
						}
					}
					else
						continue;
			...
			{
				if (trie[letters[i]] != undefined)
				{
					if (trie[letters[i]].$)
					{
						// isWord
						results.push(word + letters[i]);
					}
					// isPrefix
					this._validWords(letters.pop(i), word + letters[i], results, trie[letters[i]], withoutAsterix);
				}
				else
					continue;
	...
	prefix : function(str, trie)
	{
		...
		if (trie.$)
			return {isWord: true, isPrefix: true, trie: trie };
		return { isWord: false, isPrefix: true, trie: trie };
	},
